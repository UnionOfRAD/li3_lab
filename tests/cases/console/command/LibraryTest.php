...
namespace li3_lab\tests\cases\console\command;

use Phar;
use li3_lab\extensions\command\Library;
...
class LibraryTest extends \lithium\test\Unit {
	...
	public function testExtract() {
		...
		$lab = Libraries::get('li3_lab', 'path');

		$path = '/extensions/command/create/template/app.phar.gz';
		$expected = "library_test created in {$this->_testPath} from ";
		$expected .= realpath($lab . $path) . "\n";
	...
	public function testExtractAndReplace() {
		...
		$lab = Libraries::get('li3_lab', 'path');
		$path = '/extensions/command/create/template/test-app-replacements.phar.gz';
	...
	public function testExtractWhenLibraryDoesNotExist() {
		...
		$lab = Libraries::get('li3_lab', 'path');
		$tplPath = realpath($lab . '/extensions/command/create/template');
	...
	public function testExtractPlugin() {
		...
		$lab = Libraries::get('li3_lab', 'path');
		$target = '/extensions/command/create/template/plugin.phar.gz';
		$expected .= realpath($lab . $target) . "\n";
		$result = $this->library->response->output;
		$this->assertEqual($expected, $result);
